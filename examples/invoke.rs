...
use recue_wasmi::{ImportsBuilder, ModuleInstance, NopExternals, RuntimeValue};
...
fn main() {
  ...
  let loaded_module =
    recue_wasmi::Module::from_parity_wasm_module(module).expect("Module to be valid");
  ...
  println!(
    "Result: {:?}",
    main
      .invoke_export(func_name, &args, &mut NopExternals)
      .expect("")
  );
}
