...
fn run_spec(
  data: &[u8],
  stdout_msg_buf: &mut [u8; 64],
  stderr_msg_buf: &mut [u8; 64],
) -> Result<(), ()> {
...

  {
    ...
    seedfile
      .write_all(data)
      .expect("write fuzzer input to temporary file");
    seedfile
      .flush()
      .expect("flush fuzzer input to temporary file before starting wasm-opt");
  }
